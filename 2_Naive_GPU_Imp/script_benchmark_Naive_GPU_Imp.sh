...
echo -e "${BLUE}🔄 Installing CUDA extension (setup.py)...${NC}"
echo -e "${YELLOW}⏳ This may take a few minutes, especially during initial compilation...${NC}"
python ../setup.py install > /dev/null 2>&1

if [[ $? -ne 0 ]]; then
    echo -e "${RED}✖ setup.py install failed${NC}"
    exit 1
else
    echo -e "${GRAY}✔ Extension installed successfully.${NC}"
fi
